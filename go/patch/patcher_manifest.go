...
func (p *Patcher) replaceManifestField(patch Patch) error {
	...
	case "Payees":
		...
		if len(patch.Path.Fields) == 1 {
			...
			chainID, ethAddr, err := pathToChainIDAndAddress(patch.Path.Fields)
			if err != nil {
				return fmt.Errorf("invalid payees path: %w", err)
			}
	...
	case "ShippingRegions":
		...
		if len(patch.Path.Fields) == 1 {
			...
			regionName, ok := patch.Path.Fields[1].(string)
			if !ok {
				return fmt.Errorf("invalid region name: %T", patch.Path.Fields[1])
			}
	...
	case "AcceptedCurrencies":
		...
		if len(patch.Path.Fields) == 1 {
			...
			chainID, ethAddr, err := pathToChainIDAndAddress(patch.Path.Fields)
			if err != nil {
				return fmt.Errorf("invalid acceptedCurrencies path: %w", err)
			}
...
func (p *Patcher) addManifestField(patch Patch) error {
	...
	case "Payees":
		...
		if _, exists := p.shop.Manifest.Payees[chainID][ethAddr]; exists {
			return fmt.Errorf("payee %s already exists for chain %d", ethAddr.Hex(), chainID)
		}
	...
	case "AcceptedCurrencies":
		...
		if _, exists := p.shop.Manifest.AcceptedCurrencies[chainID][ethAddr]; exists {
			return fmt.Errorf("currency %s already exists for chain %d", ethAddr.Hex(), chainID)
		}
