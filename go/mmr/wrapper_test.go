...
func TestWrapperPostgres(t *testing.T) {
	...
	tree, err := massmmr.NewPostgresVerifierTree(connPool, hasher, testId)
	assert.Nil(t, err)
...
func TestWrapperInMemory(t *testing.T) {
	db := massmmr.NewInMemoryVerifierTree(hashFn(), 8)
	testWrapper(t, db)
}

func testWrapper(t *testing.T, tree massmmr.VerifierTree) {
