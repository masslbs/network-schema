...
# SPDX-License-Identifier: MIT
{
  ...
  outputs = inputs @ {
    ...
    flake-parts.lib.mkFlake {inherit inputs;} {
      ...
      # Expose Python functions through flake.lib
      flake.lib = {
        # Function to create Python package overrides
        makePythonPackageOverrides = {pkgs}: let
          ...
          pyunormalize = ourPython.pkgs.buildPythonPackage rec {
            ...
            pyproject = true;
            build-system = [ourPython.pkgs.setuptools];
            buildInputs = [ourPython.pkgs.setuptools];
            # le sigh...
            postUnpack = ''
        ...
        # Function to get base Python packages
        getBasePythonPackages = ps:
          with ps; [
            protobuf
            web3
        ...
        # Function to create Python environment with base packages
        makePythonEnvironment = {
          pkgs,
          additionalPackages ? [],
        }: let
          baseOverrides = inputs.self.lib.makePythonPackageOverrides {inherit pkgs;};

          # Allow additional overrides to be composed
          allOverrides =
            if additionalPackages != []
            then pkgs.lib.composeExtensions baseOverrides (self: super: {})
            else baseOverrides;

          python = pkgs.python3.override {
            packageOverrides = allOverrides;
            self = pkgs.python3;
          };
        in
          python.withPackages (
            ps:
              inputs.self.lib.getBasePythonPackages ps ++ additionalPackages ps
          );
      };
      ...
      perSystem = {
        ...
        # Use the lib functions for consistency
        packageOverrides = inputs.self.lib.makePythonPackageOverrides {inherit pkgs;};
        ...
        mass-python = inputs.self.lib.makePythonEnvironment {
          inherit pkgs;
          additionalPackages = ps: []; # No additional packages in base flake
        };
