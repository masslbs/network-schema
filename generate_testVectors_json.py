...
newKc1 = mevents.Account(
    enroll_keycard = mevents.Account.KeyCardEnroll(
        ...
        keycard_pubkey=mtypes.PublicKey(
            raw=public_key_from_account(kc1).to_bytes()),
    )
)
...
newKc2 = mevents.Account(
    enroll_keycard = mevents.Account.KeyCardEnroll(
        ...
        keycard_pubkey=mtypes.PublicKey(
            raw=public_key_from_account(kc2).to_bytes()
        ),
    )
)
...
listing_color_and_size.options[1].CopyFrom(mtypes.ListingOption(
    id = current.id,
    variations=[v for v in current.variations if v.id != l2_color_red],
))
current = listing_color_and_size.stock_status
for found in [ss for ss in current if l2_color_red in ss.variation_ids]:
    listing_color_and_size.stock_status.remove(found)
...
add_to_order_open = mevents.UpdateOrder(
    ...
    add_items_to_order=[order_open_item],
)
...
add_to_order_paid = mevents.UpdateOrder(
    ...
    add_items_to_order=[order_paid_item],
)
...
add_to_order_canceled = mevents.UpdateOrder(
    order_id = order_canceled.id,
    add_items_to_order=[order_canceled_item],
)
...
add_to_order4 = mevents.UpdateOrder(
    order_id=order4.id,
    add_items_to_order=[change4_1, change4_2],
)
