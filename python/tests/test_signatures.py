...
def test_optional_fields():
    ...
    events = [
        ...
        (
            mevents.ShopEvent(
                update_listing=mevents.UpdateListing(
                    listing_id=test_id, base_price=test_price
                )
            ),
        ...
        (
            mevents.ShopEvent(
                update_order=mevents.UpdateOrder(
                    order_id=test_id, canceled=mevents.UpdateOrder.Canceled()
                )
            ),
        ...
        (
            mevents.ShopEvent(
                change_inventory=mevents.ChangeInventory(listing_id=test_id, diff=1)
            ),
        ...
        (
            mevents.ShopEvent(
                change_inventory=mevents.ChangeInventory(
                    listing_id=test_id, diff=1, variation_ids=[1, 2, 3]
                )
            ),
