...
from massmarket_hash_event.cbor import Manifest, Payee, ShippingRegion, PriceModifier, ModificationAbsolute
...
def test_cbor_manifest_cbor_keys():
    ...
    manifest = Manifest(
        ...
        pricing_currency=ChainAddress(
            chain_id=1337,
            address="0xff00ff00ff00ff00ff00ff00ff00ff00ff00ff00"
        ),
        shipping_regions={
...
def test_cbor_manifest_from_vectors_file():
    ...
    file_path = os.path.join(os.path.dirname(__file__), "..", "..", "vectors", "ManifestOkay.json")
    ...
    def verify_manifest_snapshot(snapshot):
        ...
        assert manifest_obj.payees["default"].call_as_contract == expected_payee["CallAsContract"]

        # Check accepted currencies
        assert len(manifest_obj.accepted_currencies) == len(expected["AcceptedCurrencies"])
        for actual, expected_curr in zip(manifest_obj.accepted_currencies, expected["AcceptedCurrencies"]):
            expected_chain_addr = ChainAddress(
                chain_id=expected_curr["ChainID"],
                address=expected_curr["Address"]
            )
        ...
        assert actual_region.price_modifiers == expected_region["PriceModifiers"]  # Both should be None
